...
//-------------------------------------------------------------------------------------
inline int swapNodesInKempeChain(vector<vector<int>>& theColours,
                                 vector<vector<int>>& sol,
                                 vector<vector<int>>& theGroups, int group0,
                                 int group1) {
  // Having Identified the Kempe chain, this operator now swaps the colors of
  // the relevant nodes in the two colour classes. It does this by making 2 new
  // vectors and replacing the old ones
  int i, blackCnt = 0;
  ...
  // Now replace the relavant parts of the candidate solution with these new
  // vectors
...
//-------------------------------------------------------------------------------------
inline void DFSVisit(int uPos, int theGroup, Graph& g,
                     vector<vector<int>>& theColours,
                     vector<vector<int>>& theGroups) {
...
//-------------------------------------------------------------------------------------
inline int performKempeChainInterchange(vector<vector<int>>& sol, Graph& g,
  ...
  // Kempe chain by seeing which nodes are BLACK. (note it could be that all
  // nodes are black ==> colour relabelling. This serves no purpose, but oit is
  // carried out anyway in this case.
...
void doRandomPeturbation(vector<int>& osp, int k, Graph& g) {
  ...
  // Procedure that randomly alters a partial, proper g.col solution. First
  // convert to parition form
  for (i = 0; i < g.n; i++) {
  ...
  for (i = 0; i < k * 2; i++) {
    // Do a random Kempe chain move. First select a random group and a random
    // vertex within it
    do {
