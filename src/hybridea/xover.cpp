...
inline void chooseParents(int popSize, vector<int>& parents) {
  int i, r;
...
inline int intersection(vector<int>& A, vector<int>& B) {
  int i = 0, j = 0, inter = 0;
...
inline void removeConflicts(vector<int>& sol, Graph& g) {
  int i, j, r;
...
/*********************************************************************************************************************************/
// Procedures for MultiParentXover. If numParents is set to 2 then we get GPX
// crossover in its original form
/*********************************************************************************************************************************/
inline void copyColour(int x, int y, int col, vector<vector<int>>& parentCpys,
                       vector<int>& osp, vector<vector<int>>& parentCard,
                       Graph& g) {
  // This copies all nodes with colour x in parent y into osp, labelling them as
  // colour col.
  int i, j;
  for (i = 0; i < g.n; i++) {
    if (parentCpys[y][i] == x) {
...
int chooseBiggestCol(vector<vector<int>>& parentCard, vector<int>& tabuList,
                     int& x, int& y, int currentCol, int k) {
  // Choose entry in the rows of the array that are available with the biggest
  // value (break ties randomly) x refers to the colour chosen, y the parent it
  // is in.
  int i, p, maxCard = -100;
  ...
  if (xList.empty()) {
    ...
    // We now have a list of valid choices for the next colour class. Choose one
    // at random;
    i = rand() % xList.size();
    x = xList[i];
    y = yList[i];
...
inline void multiParent(vector<int>& osp, vector<int>& parents, Graph& g, int k,
                        vector<vector<int>>& population, bool doKempeMutation) {
  int i, col, x, y, numUnplaced = 0, j;
  int numParents = parents.size();
  int Q = numParents / 2;

  // Keeps a record at for when parents can be considered for colour transfer.
  // Initially all parents are available
  ...
  // Now copy the parents, and calculate their colour cardinality arrays
  // (remember cols go from 1 up to k inclusive)
  ...
  for (col = 1; col <= k; col++) {
    // We build up colour col. First choose the biggest colour x from an
    // available parent y (break ties randomly)
    chooseBiggestCol(parentCard, tabuList, x, y, col, k);
    // Update tabuList. Parent y cannot be considered for the next Q colours
    tabuList[y] = col + Q;
    // Copy colour across to the offspring osp (assuming a colour was found) and
    // remove these vertices from all parents
...
/*********************************************************************************************************************************/
void matchP1toP2(vector<vector<int>>& p1Copy, vector<vector<int>>& p2Copy,
                 int k) {
  ...
  // Make the matrix a minimisation matrix and then find the optimal matching
  // via hungarian
  for (i = 0; i < k; i++)
    for (j = 0; j < k; j++)
  ...
  for (i = 0; i < k; i++)
    p1Copy[i].swap(temp[matching[i]]);
  p1Copy.swap(temp);
}
inline void randomlyPermuteGroups(vector<vector<int>>& sol) {
  int i, ran;
...
void GGA(vector<int>& osp, vector<int>& parents, Graph& g, int k,
         vector<vector<int>>& population) {
  int i, j, x1, x2, numUnplaced = 0, numInjected = 0, p1 = parents[0],
                    p2 = parents[1];
  // Make group representation copy of P1 and p2
  vector<vector<int>> p1Copy(k, vector<int>()), p2Copy(k, vector<int>()),
      temp(k, vector<int>());
  // Do this by simply copying the information directly from the parents in the
  // population
  for (i = 0; i < g.n; i++) {
  ...
  // Choose 2 crossoverpoints x1<=x2. All groups between x1 (inclusive),...,x2
  // (not inclusive) inclusive will come from p2;
  x1 = rand() % (k);
  x2 = rand() % (k + 1);
  if (x1 > x2) swap(x1, x2);
  ...
  // Now copy in information from p2Copy, possibly overwriting stuff that came
  // from p1
  for (i = x1; i < x2; i++) {
    for (j = 0; j < p2Copy[i].size(); j++)
      osp[p2Copy[i][j]] = i + 1;
...
/*********************************************************************************************************************************/
inline void relabelMaxMatch(vector<int>& p1, vector<int>& p2, Graph& g, int k) {
  int i, j, maxVal = INT_MIN;
  ...
  for (i = 0; i < k; i++)
    for (j = 0; j < k; j++)
      matrix[i][j] = maxVal - matrix[i][j];
  doHungarian(k, matrix, matching);
  // Relabel the groups in p1 to maxMatch those of p2 (remember, groups are
  // labelled 1,...,k in population)
  for (i = 0; i < g.n; i++)
    p1[i] = matching[p1[i] - 1] + 1;
}
inline void nPointX(vector<int>& osp, vector<int>& parents, Graph& g, int k,
                    vector<vector<int>>& population) {
  double xrate = 0.5;
...
void doCrossover(int xOverType, vector<int>& osp, vector<int>& parents,
  ...
  if (xOverType == 1)
    ...
    multiParent(osp, parents, g, k, population, true);
  else if (xOverType == 3)
    multiParent(osp, parents, g, k, population, false);
  else if (xOverType == 4)
    GGA(osp, parents, g, k, population);
  else if (xOverType == 5)
    nPointX(osp, parents, g, k, population);
  else {
    cout << "XOver specified does not exist\n";
    exit(1);
  }
}
} // namespace hybridea
