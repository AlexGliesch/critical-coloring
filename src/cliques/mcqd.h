...
#endif
  Maxclique(const bool* const*, const int, timer , const float = 0.025);
  timer tmr;
...
Maxclique::Maxclique(const bool* const* conn, const int sz, timer tm, const float tt)
    : pk(0), level(1), Tlimit(tt), V(sz), Q(sz), QMAX(sz), tmr(tm) {
  assert(conn != 0 && sz > 0);
  for (int i = 0; i < sz; i++)
    V.push(i);
  e = conn;
...
void Maxclique::cut2(const Vertices& A, Vertices& B) {
  for (int i = 0; i < A.size() - 1; i++) {
    if (connection(A.end().get_i(), A.at(i).get_i())) B.push(A.at(i).get_i());
  }
}
...
void Maxclique::expand(Vertices R) {
  while (R.size()) {
    if (tmr.timed_out()) throw std::logic_error("timed out");
    if (Q.size() + R.end().get_degree() > QMAX.size()) {
      ...
      if (Rp.size()) {
        ...
        /*std::cout << "step = " << pk << " current max. clique size = " << Q.size()
                  << std::endl;*/
        QMAX = Q;
      }
      Rp.dispose();
